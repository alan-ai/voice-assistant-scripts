...
const ALL_PRODUCTS = project.products
...
question(
  ...
  (p) => {
    p.play(
      "This is the Flutter Shrine demo app. (With a visual voice experience powered by Alan Platform.|)"
    );
...
question("What is Alan (AI|Platform|)", (p) => {
  p.play(
    "Alan AI is a Voice AI platform that lets you add a (complete|) visual voice experience to any application. (The voice in this application is powered by Alan AI.|)"
  );
});

question("Do you have $(R* .+)", "I want $(R* .+)", (p) => {
...
question(
  ...
  (p) => {
    if (!p.visual.screen) {
      p.play(
        "This is a sample Flutter app powered by Alan Platform. Unfortunately, I am not able to say what screen you are on"
      );
      return;
    }
    switch (p.visual.screen) {
      ...
      case "cart":
        p.play(
          "This is your cart. Here you can ask questions about your order, continute shopping or go to checkout"
        );
        break;
    }
  }
