...
intent(
  "(change|update|set) $(L home|office|work) (address|location) to $(LOC)",
  reply(
    "do you want to set $(L) address to $(LOC)",
...
intent(
  "(remember|set|update) (this|current|that) (location|address|) as $(L home|office|work) (address|)",
  (p) => {
    p.play("set");
  }
);
...
let where = context(() => {
  ...
  follow("$(LOC)", (p) => p.resolve(p.LOC.value));

  follow("never mind", "(cancel|forget) (that|)", (p) => p.resolve(null));
...
let whatAddress = context(() => {
  ...
  follow("(it's|the address is|) $(LOC)", (p) => {
    p.state.push(p.LOC.value);
    checkAddress(p);
  });

  follow("never mind", "(cancel|forget) (that|)", (p) => {
    p.resolve(null);
  });

  fallback(
    "(Please|Just|) tell me the (address|location)",
...
intent(
  "navigate (me|)",
  "build (me|) (a|) route (for me|)",
  reply("where (|to)?", async (p) => {
...
intent("(navigate me|build route|get me) (to|) $(LOC)", (p) => {
  p.play(`navigating to ${p.LOC}`);
});

intent(
  "(navigate|take me|get me|build route) (to|) (my|) $(L home|office|work|gym)",
