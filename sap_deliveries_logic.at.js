...
intent(
  `(go to|open|show) $(SCREEN ${Object.keys(screenAliases).join(
    `|`
  )}) (screen|)`,
  (p) => {
    ...
    if (screen && screen !== "Main") {
      p.play({
        command: "navigate",
        screen: screenAliases[p.SCREEN.toLowerCase()],
      });
    }
  }
);
...
intent(
  "(What|Which) (are the|) products (are|) (available|there|do you have)",
  (p) => {
    if (!_.isEmpty(project.prods)) {
      p.play({ command: "navigate", screen: "Product" });
      p.play(
        `There are (several|${_.size(
          project.prods
...
intent(
  `(show|open) (the|) $(P ${project.prods
    .map((s) => s.Name)
    .join(`|`)}) (card|details|screen|)`,
  (p) => {
...
function findProductByName(name) {
  return _.first(
    project.prods.filter(
      (pr) => pr.Name && pr.Name.toLowerCase() === name.toLowerCase()
    )
  );
}
...
intent(
  `How much (is|does) the $(P ${project.prods
    .map((s) => s.Name)
    .join(`|`)}) (cost|)`,
  `What is the (cost|price) of the $(P ${project.prods
    .map((s) => s.Name)
    .join(`|`)})`,
  (p) => {
    let product = findProductByName(p.P.value);
...
intent(
  `Who (supplies|provides|has) the $(P ${project.prods
    .map((s) => s.Name)
    .join(`|`)})`,
  `Who is the $(P ${project.prods
    .map((s) => s.Name)
    .join(`|`)}) (supplier|provider)`,
  (p) => {
    let product = findProductByName(p.P.value);
    if (!product) {
      p.play(`${p.P} not found in our product list`);
      ...
      if (!sup) {
        ...
        p.play(
          `The ${p.P} is supplied by ${
            supById[product.SupplierId].SupplierName
          }`,
...
intent(
  "(What|Which) (are the|) (product|) categories (are|) (available|there|do you have)",
  (p) => {
...
const findProductsInCategoryByPrice = [
  `(show|find) $(CAT ${Object.keys(categoryAliases).join(
    `|`
  )}) (that cost|with a price of|) $(COMP ${Object.keys(compAliases).join(
    `|`
  )}) than $(NUMBER) $(CUR dollar|euro|dollars|euros)`,
  `(what|how many) $(CAT ${Object.keys(categoryAliases).join(
    `|`
  )}) do you have (that cost|with a price of|) $(COMP ${Object.keys(
    compAliases
  ).join(`|`)}) than $(NUMBER) $(CUR dollar|euro|dollars|euros)`,
  `(what|how many) $(CAT ${Object.keys(categoryAliases).join(
    `|`
  )}) (that cost|with a price of|) $(COMP ${Object.keys(compAliases).join(
    `|`
  )}) than $(NUMBER) $(CUR dollar|euro|dollars|euros) do you have`,
  `do you have $(CAT ${Object.keys(categoryAliases).join(
    `|`
  )}) (that cost|with a price of|) $(COMP ${Object.keys(compAliases).join(
    `|`
  )}) than $(NUMBER) $(CUR dollar|euro|dollars|euros)`,
...
intent(`do you have $(CAT ${Object.keys(categoryAliases).join(`|`)})`, (p) => {
  categoryIntent(
    p,
    (len, cat) => `Yes. There are ${len} ${cat}`,
    (cat) => `No. There are no ${cat}`
  );
});

intent(
  `how many $(CAT ${Object.keys(categoryAliases).join(`|`)}) (do you have|)`,
  (p) => {
    categoryIntent(
      p,
      (len, cat) => `We have ${len} ${cat}`,
      (cat) => `We don't have ${cat}`
    );
  }
);
...
intent("What can (I|you) do", "What's this (SAP|) (app|application)", (p) => {
  p.play(
    "(This is the SAP Delivery Demo App. With a visual voice experience powered by Alan AI|) " +
...
intent("What is Alan (AI|)?", (p) => {
  p.play(
    "Alan AI is a Voice AI platform that lets you add a (complete|) visual voice experience to any application. " +
...
intent(
  ...
  (p) => {
    ...
    switch (p.visual.screen) {
      case "Main":
        p.play(
          "This is the Collections screen. Here, you can use commands like What products do you have?, " +
      ...
      case "Supplier":
        p.play(
          "This is the Suppliers screen. Here, you can (use commands|ask questions) like Who are the suppliers? " +
      ...
      case "SupplierDetails":
        p.play(
          "This is the supplier details screen. (Here,|) you can (use commands|ask questions) like " +
      ...
      case "ProductCategory":
        p.play(
          "This is the Product Category screen. (Here,|) you can (use the command|ask the question) What are the categories?"
        );
        break;

      case "PurchaseOrderHeaders":
        p.play(
          "This is the Purchase Order Header screen. (Here,|) you can (use commands|ask questions) like " +
      ...
      case "Product":
        p.play(
          "This is the Products screen. (Here,|) you can (use commands|ask questions) like What products are available? " +
            `How much is the ${rand(
              project.prods.map((p) => p.Name)
            )}?, and What ${rand(
      ...
      case "ProductDetails":
        p.play(
          "This is the product details screen. (Here,|) you can (use commands|ask questions) like How much is it? " +
...
function findProductsByIntentParams(categoryP, comparatorP, field, valueP) {
  ...
  return project.prods.filter(
    (p) => p.Category === category && comparator(parseInt(p[field]), value)
  );
}
...
function conditionIntent(p, foundAnswer, notFoundAnwer) {
  ...
  const conditionStr = `${p.CAT} ${p.COMPs.join(" ")} than ${p.NUMBER} ${
    p.CUR || ""
  }`;
  p.play(
    products.length
