...
const whatLanguage = context(() => {
  ...
  follow(`(What|Which) languages (are available|can I use)`, (p) => {
    p.play(
      `(You can translate|The available languages are) ${joinAnd(
  ...
  fallback(
    "Please, say, what is destination language?",
    "To what langauge?",
    "Specify the language for the translation"
  );
});
...
intent(
  `Can (you|I) translate to $(L ${LANGS_INTENT})`,
  `(Can you|Do you) (understand|speak|translate to) $(L ${LANGS_INTENT})`,
  async (p) => {
    let lang = languages[p.L.toLowerCase()];
    if (lang) {
      ...
      p.play(
        `No, sorry, I can only translate ${joinAnd(Object.keys(languages))}`
      );
    }
  }
);
...
intent(
  `Translate $(W* .+) (in|)to $(L ${LANGS_INTENT})`,
  `(How|) (do you|would you|to|) say $(W* .+) (in|on) $(L ${LANGS_INTENT})`,
  `What is $(W* .+) in $(L ${LANGS_INTENT})`,
  (p) => {
    let lang = languages[p.L.toLowerCase()];
    if (!lang) {
      p.play(
        `I can't translate this to ${p.L}. Please choose another language`,
...
intent(`(What|Which) languages (are available|can I use)`, (p) => {
  p.play(
    `You can translate English to ${joinAnd(
      Object.keys(languages).map(capitalize)
    )}`
  );
});
...
intent("Help (me|)", "What (to|can I) do (here|)", (p) => {
  p.play(
    `I can translate ${joinAnd(
      Object.keys(languages)
    )}. For example, you can ask me how to translate (Hello|Artificial Intelligence|Good day) to (${Object.keys(
      languages
    ).join(`|`)})`
  );
});
    ...
    (response) => {
      if (!response.error) {
        p.play({
          command: "showTranslation",
          srcLang: "English",
          dstLang: _.invert(allLang)[lang],
          srcText: text,
          dstText: response.translated,
          embeddedPage: true,
          page: "translation.html",
        });
