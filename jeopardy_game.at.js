...
project.globalPrices = Object.keys(
  project.data.reduce((map, question) => {
...
function initRandomCategories() {
  ...
  while (categories.length < 4) {
    if (project.data[i]) {
      ...
      if (
        quesiton.round === "Jeopardy!" &&
        !categories.includes(category) &&
        isQuestionNew(quesiton)
      ) {
...
function getQuestionTree(initedCategories) {
  return project.data.reduce((tree, question) => {
    if (question.value && isQuestionNew(question)) {
      ...
      if (
        initedCategories.includes(categoryName) &&
...
async function startRound(p) {
  ...
  if (!questions) {
    ...
    const question =
      questions[Math.round(Math.random() * Math.floor(questions.length - 1))];
...
const roundContext = context(() => {
  follow(
    "(what is|what are|who is|who are|) (the|) $(ANSWER~ u:answer)",
    (p) => {
      endRound(p, true);
    }
  );
...
const categoryContext = context(() => {
  follow(
    "$(CATEGORY~ u:categoryIntent) (for|) $(PRICE p:globalPrices)",
    async (p) => {
  ...
  follow("$(CATEGORY~ u:categoryIntent)", async (p) => {
    ...
    p.userData.categoryPrices = getPriceCategoryIntent(
      p.userData.questionTree[category]
    );
...
const priceContext = context(() => {
  ...
  fallback("Please select the value for your question");
});
