...
const CATEGORY_ALIASES = _.reduce(
  Object.keys(menu),
  (a, p) => {
...
intent(
  ...
  `where am I`,
  reply(
    "(This is|It's) (just|simple) Food Ordering example application for (food delivery service|pizza ordering)"
  )
);

intent(
  "What (kind|types|sort) of food do you have (to order|)",
...
intent(
  ...
  (p) => {
    ...
    switch (route) {
      ...
      case "/menu/drink":
        p.play(
          "Here you can navigate through the menu and add and remove food to your order. To open menu category say (Open|go to) (drinks|pizza|street food|desserts). " +
      ...
      case "/cart":
        p.play(
          "You are in your cart. You should answer questions about delivery address and time. " +
      ...
      case "/finish-order":
        p.play(
          "You finished your order, if you want to make another order say 'go back' or 'open menu' and add new items to your order"
        );
        break;
      case "/cleared-order":
        p.play(
          "You have (cleared|canceled) your order, if you want to make another order say 'go back' or 'open menu' and add new items to your order"
        );
        break;
...
intent(
  ...
  reply(
    "Choose food category and add items from menu to order. For example, you can say:" +
...
intent(
  `(open|what do you have in|choose|select|) $(ITEM ${CATEGORY_LIST})`,
  (p) => {
    p.play({
      command: "navigation",
      route: `/menu/${CATEGORY_ALIASES[p.ITEM.toLowerCase()]}`,
    });
...
let confirm = context(() => {
  ...
  follow(
    ...
    (p) => {
      p.play({ command: "navigation", route: "/cart" });
      p.play(
        "OK, please (make neccessary corrections|update an order|fix what you want)"
      );
      p.resolve(null);
    }
  );
...
let playDelivery = function (p, address, date, time) {
  if (!address) {
    ...
    p.play({ command: "navigation", route: "/cart" });
    p.play(
      `OK, your order will be delivered to ${address}. Do you want to confirm your order?`
    );
...
intent(`(add|I want|do you have|order) $(F ${DISHES_INTENT})`, (p) => {
  p.play(
    `Unfortunately you can't add  ${p.F} to your order. But we can offer it in our restaurant`
  );
});
...
function addItems(p, items, shift, pos = []) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!id) {
      ...
      let number =
        p.NUMBERs && p.NUMBERs[i - shift] ? p.NUMBERs[i - shift].number : 1;
      if (!_.isEmpty(pos)) {
...
// replace items
intent(
  `change (one of |) the $(ITEM ${ITEMS_INTENT}) to a $(ITEM ${ITEMS_INTENT})`,
  (p) => {
...
intent(
  `(how much|what) does $(ITEM ${ITEMS_INTENT}) cost`,
...
projectAPI.greet = (p, param, callback) => {
  p.play(
    "Welcome to the Food Ordering demo app for food delivery. (How can I help you|What can I get for you|May I take your order|What would you like to order)?"
  );
};
