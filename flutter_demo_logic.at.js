...
onCreateProject(() => {
  project.itemsIntent = project.products
    .map((p) => `${p.name}_~${p.id}`)
    .join("|");
  ...
  Object.keys(project.categoryTypes).forEach((categoryName) => {
    ...
    const categoryIntent = [...category]
      .map((type) => `${type}_~${type}`)
      .join("|");
  ...
  project.categoryIntents = Object.keys(project.categoryAliases)
    .reduce((intent, categoryId) => {
      const categoryNames = project.categoryAliases[categoryId]
        .map((_) => `${_}_~${categoryId}`)
        .join("|");
...
intent("(Show me|show|open|go to) (menu|main menu)", "main menu", (p) => {
  p.play({
    command: "navigation",
    route: "/menu",
  });
...
intent(
  "(add|I want|order|get me) (few|some) $(ITEM p:itemsIntent)",
  async (p) => {
...
intent(
  "change (one of|) (the|) $(ITEM p:itemsIntent) to (a|) $(ITEM p:itemsIntent)",
  (p) => {
    ...
    if (!idForRemoval || !idForAdding) {
      ...
      const removeQuantity =
        p.NUMBER_ && p.NUMBER_[0] ? p.NUMBER_[0].number : 1;
      ...
      p.play(
        `Removed ${removeQuantity} ${p.ITEM_[0]} and added ${addQuantity} ${p.ITEM_[1]}`
      );
    }
    p.play({
      command: "navigation",
...
intent(
  "(my order|my cart|open cart|show cart|show my cart|order details|order info)",
  (p) => {
    const order = p.visual.order;
    if (_.isEmpty(order)) {
...
intent(
  ...
  (p) => {
    const type = p.TYPE.label;
    const filteredProducts = project.products.filter(
      (i) => i.type === type && i.price <= p.NUMBER.number
    );
    if (_.isEmpty(filteredProducts)) {
      p.play(
        `We could not find ${type} cheaper than ${p.NUMBER.number} dollars`
      );
      return;
    }
    p.play({
    ...
    p.play(
      `We (offer|have) several ${type} (under|cheaper than|for less than) ${p.NUMBER.number} dollars:`
    );
...
intent("(Clear|Cancel) (my|) order", "(Clear|flush|empty) (my|) cart", (p) => {
  p.play({
    command: "clearOrder",
    route: "cleared-order",
  });
  ...
  follow(
    ...
    async (p) => {
      ...
      if (p.ITEM) {
        ...
        const filteredProducts = project.products.filter(
          (i) => i.type === p.TYPE.label && i.category === p.state.category
        );
        highlightProductList(p, filteredProducts, true);
      } else if (p.CATEGORY) {
...
const confirm = context(() => {
  ...
  follow("(no|change|invalid|nope|not correct|stop|back)", (p) => {
    ...
    p.play(
      "OK, please (make necessary corrections|update an order|fix what you want)"
    );
    p.resolve(null);
  });
});
...
const howMany = context(() => {
  ...
  follow(
    "(Cancel|abort|that is not what I wanted|no|zero|none|go back|back)",
    (p) => {
      p.resolve(null);
    }
  );
...
function playCategoryList(p, category) {
  ...
  p.play(
    `Which ${p.CATEGORY} would you like? We have several types available:`
  );
...
async function addItems(p, productList, productQuantities, offset) {
  ...
  if (
    ...
    p.play(
      `You requested ${p.state.quantity} ${
  ...
  if (!p.state.isFullResponseReceived) {
    ...
    answer.push(
      "(Something else?|What else can I do for you?|Do you want to checkout?|Do you need anything else?|)"
    );
  }
  p.play(answer.join(" "));
