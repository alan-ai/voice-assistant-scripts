...
let dsi = _.flatten(
  _.map(discreteSentiments, (s) => _.map(s, (e) => `${e}~${s[0]}`))
).join("|");
...
let oneToTenQuestion = context(() => {
  follow(
    `(I would rate it|Rating|it's|) $(NUMBER) (because|since|as|) $(I* (.*))`,
    (p) => {
      if (p.NUMBER.number < 1 || p.NUMBER.number > 10) {
        p.play(
          `(Sorry|Try again), your rating must be (a number|) between one and ten.`
        );
      } else {
        p.resolve({
          cmd: "ok",
          res: {
            ...
            answer: `Rating <b>${p.NUMBER}</b> and feedback <i>"${p.I}"</i>`,
            feedback: p.I.value,
          },
        });
      }
  ...
  follow(`(I would rate it|Rating|it's|) $(NUMBER)`, (p) => {
    if (p.NUMBER.number < 1 || p.NUMBER.number > 10) {
      ...
      p.resolve({
        cmd: "ok",
        res: { answer: `Rating <b>${p.NUMBER}</b>`, rating: p.NUMBER.number },
      });
    }
  });
  fallback(
...
let durationQuestion = context(() => {
  ...
  follow(
    `$(A couple|few|many|barely|a long time|not so long|several) (of|any|) $(T year|day|month|week|years|days|months|weeks|)`,
    (p) => {
...
let likelihoodQuestion = context(() => {
  follow(
    `(It's|I find it|It is|I am|I feel|I think|) $(T~${dsi}) (because|since|as|) $(I* (.*)) `,
    (p) => {
      if (p.I.length) {
        p.resolve({
          cmd: "ok",
          res: { answer: `<b>${p.T.label}</b> - <i>${p.I}</i>` },
        });
      } else {
  ...
  fallback(
    `(Sorry, didn't recognize that.|) (To rate your experience,|) just say any of the following (descriptors|). ${hint}.`
  );
});
...
function navigation() {
  ...
  follow("(Stop|Exit|End|Never Mind|Leave|Terminate|Quit) (survey|)", (p) =>
    p.resolve({ cmd: "exit" })
  );
  follow(
    "(Please|) (go to|open|return to|) (the|) (previous|previous question|before|go back|back)",
    (p) => p.resolve({ cmd: "back" })
  );
  follow(
    "(Please|) (go forward|skip|skip question|skip this question|next|next question|pass|I don't know)",
    (p) => p.resolve({ cmd: "skip" })
  );
  follow("(go to|open|return to|) (the|) $(ORDINAL) (question|item)", (p) =>
    p.resolve({ cmd: "goto", res: p.ORDINAL.number })
  );
  follow("(go to|open|return to|) (question|item) $(NUMBER)", (p) =>
    p.resolve({ cmd: "goto", res: p.NUMBER.number })
  );
  follow("(go to|open|return to|) (the|) last (question|item)", (p) =>
    p.resolve({ cmd: "goto", res: -1 })
  );
  follow("How many questions are in (this|the|) survey", (p) =>
    p.resolve({ cmd: "numQ" })
  );
  follow("How many questions are left (in this survey|in the survey|)", (p) =>
    p.resolve({ cmd: "numQleft" })
  );
}
...
intent(`(Start|Commence|Begin|Take|) (survey|)`, async (p) => {
  ...
  while (index < project.questions.length) {
    ...
    if (cmd === "back") {
      ...
      p.play(
        `There are ${project.questions.length - index - 1} questions left`
      );
    } else {
      p.play({
        embeddedPage: true,
...
intent(`Thanks Alan`, reply(`You're welcome`));

projectAPI.greet = (p, param, callback) => {
  p.play(
    "Hi, welcome to the Customer Voice Survey. To (get started|), say '(Begin|Start|Take) survey'."
  );
};
